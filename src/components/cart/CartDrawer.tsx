      ...
      <div className="fixed top-0 left-0 h-full w-full max-w-md bg-white z-50 shadow-2xl flex flex-col animate-in slide-in-from-left duration-300">
        ...
        <div className="flex items-center justify-between p-5 border-b-2 border-[#E8EAED] bg-gradient-to-r from-[#2C2420] via-[#3d3530] to-[#2C2420]">
          ...
          <button
            type="button"
            onClick={() => setIsCartOpen(false)}
            className="text-white hover:bg-white/20 p-2.5 rounded-xl transition-all hover:rotate-90 duration-300 outline-none focus-visible:ring-2 focus-visible:ring-white/50"
                ...
                >
                  ...
                  <div className="flex flex-col items-end justify-between">
                    <button
                      ...
                      className="text-red-400 hover:text-red-600 hover:bg-red-50 p-1.5 rounded-lg transition-all hover:scale-110 outline-none focus-visible:ring-2 focus-visible:ring-red-500"
                    ...
                    <div className="flex items-center gap-1 bg-white rounded-full border-2 border-[#E8EAED] p-0.5 shadow-sm">
                      <button
                        ...
                        className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-red-50 hover:text-red-500 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-red-500"
                      ...
                      <div
                        title={item.quantity >= item.product.stock ? "الحد الأقصى للمخزون" : undefined}
                      >
                        <button
                          type="button"
                          onClick={() => updateQuantity(item.product.id, item.quantity + 1)}
                          disabled={item.quantity >= item.product.stock}
                          className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-emerald-50 hover:text-emerald-500 transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-inherit outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
          ...
          <div className="border-t-2 border-[#E8EAED] p-5 space-y-4 bg-gradient-to-b from-[#F8F9FA] to-white">
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => setIsCartOpen(false)}
                className="flex-1 text-[#2C2420] hover:bg-[#E8EAED] py-2.5 rounded-xl transition-colors font-medium text-sm border-2 border-[#E8EAED] outline-none focus-visible:ring-2 focus-visible:ring-[#2C2420]"
              >
                متابعة التسوق
              </button>
              <button
                type="button"
                onClick={clearCart}
                className="text-red-500 hover:bg-red-50 px-4 py-2.5 rounded-xl transition-colors text-sm font-medium border-2 border-red-100 outline-none focus-visible:ring-2 focus-visible:ring-red-500"
              >
                إفراغ 🗑️
              </button>
            </div>
