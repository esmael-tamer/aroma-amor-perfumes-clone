                ...
                >
                  ...
                  <div className="flex flex-col items-end justify-between">
                    ...
                    <div className="flex items-center gap-1 bg-white rounded-full border-2 border-[#E8EAED] p-0.5 shadow-sm">
                      ...
                      <div title={item.quantity >= item.product.stock ? 'الحد الأقصى للكمية' : ''}>
                      <button
                        type="button"
                        onClick={() => updateQuantity(item.product.id, item.quantity + 1)}
                        disabled={item.quantity >= item.product.stock}
                        className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-emerald-50 hover:text-emerald-500 transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-inherit focus-visible:ring-2 outline-none"
